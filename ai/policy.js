...
function sameWeightMap(left = {}, right = {}) {
  const leftKeys = Object.keys(left || {});
  const rightKeys = Object.keys(right || {});
  if (!leftKeys.length || leftKeys.length !== rightKeys.length) return false;
  return leftKeys.every((key) => (
    Object.prototype.hasOwnProperty.call(right, key)
    && Math.abs(finiteNumber(left[key]) - finiteNumber(right[key])) <= Number.EPSILON
  ));
}

function collapseLegacyDuplicatePersonalityWeights(policy) {
  const personalityEntries = Object.values(policy.personalities || {});
  if (personalityEntries.length !== 1) return;
  const [personality] = personalityEntries;
  if (sameWeightMap(policy.sharedWeights, personality?.weights)) {
    personality.weights = {};
  }
}
...
export function hydrateLearningPolicy(rawPolicy) {
  ...
  collapseLegacyDuplicatePersonalityWeights(policy);
  return policy;
}
...
export function trainFeatureBatch(policy, transitions, options = {}) {
  ...
  for (const transition of usable) {
    ...
    const trainPersonalityWeights = options.trainPersonalityWeights === true;

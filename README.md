...
The trainer prints live progress with episode count, speed, loss, survival/fall/truncation rates, sampled player/round mix, policy mix, action distribution, action-value bucket mix, average terminal return by behavior, average frontier troop share, income-category shares, average rounds, and transition count. It writes checkpoints to `ai/checkpoints/`, evaluates them with a small tournament, and promotes the best checkpoint to `ai/models/latest.json` instead of blindly saving the last update. Use `--checkpoint-interval 100`, `--checkpoint-eval-episodes 8`, `--training-epochs 4`, `--log-interval 25`, or `--quiet true` to tune the run.

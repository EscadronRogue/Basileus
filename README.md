...
By default, training samples a fresh mix of 3-5 player games and 6-12 round invasion decks, gives every episode its own random seed, and chooses a worker count from the machine's available CPU parallelism. It also trains for multiple epochs per collected batch, mixes learner seats against random/checkpoint/self-play opponents, and temporarily disables deal actions to reduce the court action space while the core survival policy learns. The model reward remains outcome-driven: the feature vector and diagnostics expose neutral indicators such as invasion route pressure, troop capacity, frontier/capital allocations, scoring-category shares, action costs, deal clauses, candidate context, and defender-reward context, but those indicators are not added to the learner's return. Pass `--include-deals true` to re-enable deal actions in training later. Pass `--heuristic-opponent-rate 0.25` only when you explicitly want scripted defensive opponents in the training mix. Pass `--players 4`, `--rounds 9`, `--seed 1`, or `--workers 4` when you want to pin any of those for a controlled run. You can also tune ranges with `--player-min`, `--player-max`, `--round-min`, and `--round-max`.

The trainer prints live progress with episode count, speed, loss, survival/fall/truncation rates, sampled player/round mix, policy mix, action distribution, action-value bucket mix, average terminal return by behavior, average frontier troop share, income-category shares, average rounds, and transition count. It writes checkpoints to `ai/checkpoints/`, evaluates them with a small tournament, and promotes the best checkpoint to `ai/models/latest.json` instead of blindly saving the last update. When you pass `--resume`, checkpoint numbering continues from the loaded model or latest matching checkpoint, and the resumed model is kept as the promotion baseline so a worse continuation cannot replace it. Use `--checkpoint-interval 100`, `--checkpoint-eval-episodes 8`, `--training-epochs 4`, `--log-interval 25`, or `--quiet true` to tune the run.
